...
//            }
            // ^^^ Можно проще (т.к. age в User это Integer, а не int):
    ...
    public List<User> saveAll2(List<User> allUsers) {
        String sql = "INSERT INTO users (username, email, age) VALUES (?, ?, ?)";
        try (Connection connection = dataSource.getConnection();
             PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            connection.setAutoCommit(false);

            List<List<User>> usersLists = new ArrayList<>(); // использовать chunk(allUsers)

            for (List<User> users: usersLists) {
                try {
                    for (User user : users) {
                        ps.setString(1, user.getUsername());
                        ps.setString(2, user.getEmail());
    ...
    private void transactionExample() {
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new AppException("Ошибка", e);
        }
    }
}